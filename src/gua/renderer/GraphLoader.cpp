...
namespace gua {
  ...
  // static variables
  /////////////////////////////////////////////////////////////////////////////
  unsigned GraphLoader::mesh_counter_ = 0;

  std::unordered_map<std::string, std::shared_ptr<Node>>
    GraphLoader::loaded_files_ =
    std::unordered_map<std::string, std::shared_ptr<Node>>();
  /////////////////////////////////////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////////////
  ...
  std::shared_ptr<Node> GraphLoader::load_geometry(std::string const& file_name, unsigned flags)
  {
    ...
    if (searched != loaded_files_.end()) {
      ...
      if (is_supported(file_name))
      {
        ...
        if (flags & GraphLoader::NORMALIZE_POSITION || flags & GraphLoader::NORMALIZE_SCALE) {
          auto bbox = cached_node->get_bounding_box();

          if (flags & GraphLoader::NORMALIZE_POSITION) {
            auto center((bbox.min + bbox.max)*0.5f);
            cached_node->translate(-center);
          }

          if (flags & GraphLoader::NORMALIZE_SCALE) {
  ...
  std::shared_ptr<Node> GraphLoader::create_geometry_from_file
  ...
  std::shared_ptr<Node> GraphLoader::load(std::string const& file_name,
                                       unsigned flags) {
  ...
  if (file.is_valid()) {
    ...
    if ((flags & GraphLoader::OPTIMIZE_GEOMETRY) &&
        (flags & GraphLoader::LOAD_MATERIALS)) {
    ...
    else if (flags & GraphLoader::OPTIMIZE_GEOMETRY) {
...
std::vector<GraphRessource*> const GraphLoader::load_from_buffer(char const* buffer_name,
  ...
  std::vector<GraphRessource*> meshes;

  for (unsigned int n = 0; n < scene->mNumMeshes; ++n) {
    meshes.push_back(new GraphRessource(scene->mMeshes[n], importer, build_kd_tree));
  }
...
std::shared_ptr<Node> GraphLoader::get_tree(std::shared_ptr<Assimp::Importer> const& importer,
  ...
  auto load_geometry = [&](int i) {
    ...
    GeometryDatabase::instance()->add(mesh_name, std::make_shared<GraphRessource>(ai_scene->mMeshes[ai_root->mMeshes[i]], importer, flags & GraphLoader::MAKE_PICKABLE));

    // load material
    std::string material_name("");
    unsigned material_index(ai_scene->mMeshes[ai_root->mMeshes[i]]->mMaterialIndex);

    if (material_index != 0 && flags & GraphLoader::LOAD_MATERIALS) {
    ...
    return std::make_shared<GraphNode>(mesh_name, mesh_name, material_name);
  };
...
void GraphLoader::apply_fallback_material(std::shared_ptr<Node> const& root,
