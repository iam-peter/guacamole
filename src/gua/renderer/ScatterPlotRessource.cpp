...
ScatterPlotRessource::ScatterPlotRessource(aiMesh* mesh, std::shared_ptr<Assimp::Importer> const& importer,
           bool build_kd_tree)
    : vertices_(),
      indices_(),
      vertex_array_(),
...
/*virtual*/ GeometryUberShader* ScatterPlotRessource::get_ubershader() const {
  return Singleton<ScatterPlotUberShader>::instance();
}
